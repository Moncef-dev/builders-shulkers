	...
	private static float[] vanillaBbox(ItemStack content, @Nullable Level level) {
		return VANILLA_BBOX_BY_ITEM.computeIfAbsent(content.getItem(), item -> {
			...
			for (int i = 0; i < count; i++) {
				...
				// On this branch the localTransform is our own duck field: null on a fresh vanilla probe layer
				// (identity), so the raw extents are already the vanilla-space geometry.
				Matrix4f local = ((PocketBuildContentLayer) (Object) layers[i]).shulkerInventory$getLocalTransform();
				for (Vector3fc extent : layer.shulkerInventory$getExtents().get()) {
					scratch.set(extent);
					if (local != null) {
						scratch.mulPosition(local);
					}
